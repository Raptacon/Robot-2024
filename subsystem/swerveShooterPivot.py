...
class SwerveShooterPivot(commands2.PIDSubsystem):
    ...
    def __init__(self) -> None:
        ...
        self.pidController = wpimath.controller.PIDController(1, 0, 0)
        ...
        self.handOffSwitch = self.pivotMotor.getForwardLimitSwitch(rev.SparkLimitSwitch.Type.kNormallyClosed)
    ...
    def useOutput(self, output: float, setpoint: float):
        ...
        wpilib.SmartDashboard.putNumber("Shooter Current", self.pivotMotor.getOutputCurrent())

        self.voltage = output + feedforward
        wpilib.SmartDashboard.putNumber("Shooter voltage", self.voltage)
        print(self.voltage)
    ...
    def getPostion(self) -> float:
        ...
        wpilib.SmartDashboard.putNumber("Shooter Angle Degrees", currDeg)
