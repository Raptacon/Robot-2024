...
class Drivetrain(commands2.SubsystemBase):
    ...
    kMaxVelocityMPS = 1.0
    kMaxAngularVelocityRadPS = kMaxVelocityMPS / math.hypot(
        kWheelBaseMeters / 2.0, kTrackBaseMeters / 2.0
    )
    ...
    kModuleProps = [
        {
            ...
            "channel": 50,
            "encoderCal": 83.496,
            "trackbase": kTrackBaseMeters / 2.0,
            "wheelbase": kWheelBaseMeters / 2.0,
            "inverted": False,
        },
        {
            "name": "frontRight",
    ...
    def __init__(self):
        ...
        for module in Drivetrain.kModuleProps:
            ...
            self.swerveModules.append(
                SwerveModule(
                    (trackbase, wheelbase, name),
                    channel,
                    inverted,
                    encoderCal,
                    subTable,
        ...
        self.odometry = wpimath.kinematics.SwerveDrive4Odometry(
            self.kinematics, self.getHeading(), self.moduleRotations
        )
        self.pos = Pose2d()
        self.posX = 0
        self.posY = 0
    ...
    def drive(self, xSpeed: float, ySpeed: float, rot: float, fieldRelative: bool):
        ...
        if not fieldRelative:
            ...
            chassisSpeeds = wpimath.kinematics.ChassisSpeeds.fromFieldRelativeSpeeds(
                ySpeed, -xSpeed, rot, self.getHeading()
            )
